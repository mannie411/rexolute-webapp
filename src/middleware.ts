...
export const config = {
  matcher: [
     ...
     */
    "/((?!api/|_next/|_static/|fonts/|graphics/|[\\w-]+\\.\\w+).*)",
  ],
};
...
export default async function middleware(req: NextRequest) {
  ...
  const appUrl =
    process.env.APP_ENV === "local"
      ...
      : process.env.APP_URL;
  const prodUrlWildcard = "rexolute-webapp.vercel.app";
  ...
  // if (currentHost == "app") {
  //   if (url.pathname === "/login" && isAppAuth) {
  //     url.pathname = "/";
  ...
  // if (hostname === "localhost:3000" || hostname === prodUrl) {
